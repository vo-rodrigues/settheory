...
\begin{metadefinition}\label{def:simbolos-logicos}
...
Nesse texto, seremos extremamente permissivos com a notação utilizada para os símbolos de variáveis.
...
Dito isso, há duas possíveis dúvidas que podem surgir a respeito da definição acima.
...
\begin{metadefinition}
    ...
    \begin{itemize}
        \item Em uma coleção de $\mathcal P$ de símbolos de predicado (ou relacionais), cada um com uma aridade associada.
        \item Em uma coleção de $\mathcal F$ de símbolos funcionais, cada um com uma aridade associada.
    \end{itemize}
    ...
    Juntamente com os símbolos lógicos da Definição~\ref{def:simbolos-logicos}, eles constituem o léxico total da linguagem.
\end{metadefinition}

Exemplos de símbolos de predicado não lógicos incluem $\in$ (pertencimento, comum em teoria dos conjuntos), $<$ (ordem, comum ao estudar números reais), $\triangle$ (não colinearidade entre três pontos, comum em geometria plana).
...
\begin{example}Consideremos os seguintes léxicos.\label{ex:exemplosLexicos}
    \begin{itemize}
...
Ressaltamos que, apesar de ambos variáveis e constantes serem símbolos de aridade $0$, e apesar de estarmos sendo permissivos com a notação utilizada para variáveis, estes objetos são distintos (por exemplo, apenas variáveis podem ser quantificadas!).
...
\begin{example}Utilizando a notação do Exemplo~\ref{ex:exemplosLexicos}, temos os seguintes termos.
    \begin{itemize}
        ...
        \item Considere a linguagem básica da teoria de conjuntos.
...
\begin{example}Utilizando a notação do Exemplo~\ref{ex:exemplosLexicos}, temos as seguintes fórmulas.
    \begin{itemize}
...
A partir deste ponto, deixaremos de, a menos que necessário, escrever termos e fórmulas na notação polonesa.
Seguiremos as seguintes convenções.

\begin{definition}Utilizaremos as seguintes convenções de escrita.
    Escreveremos:
\begin{itemize}
    \item $\phi \wedge \psi$ ao invés de ${\wedge}\phi\psi$;
    \item $\phi \vee \psi$ ao invés de ${\vee}\phi\psi$;
    \item $\phi \rightarrow \psi$ ao invés de ${\rightarrow}\phi\psi$;
    \item $\phi \leftrightarrow \psi$ ao invés de ${\leftrightarrow}\phi\psi$;
    \item $t_1 = t_2$ ao invés de ${=}t_1 t_2$;
    \item $P(t_1, \dots, t_n)$ ao invés de $P t_1 \dots t_n$, em que $P$ é um símbolo de predicado de aridade $n>0$ e $t_1, \dots, t_n$ são termos;
    \item $f(t_1, \dots, t_n)$ ao invés de $f t_1 \dots t_n$, em que $f$ é um símbolo funcional de aridade $n>0$ e $t_1, \dots, t_n$ são termos.
\end{itemize}
...
\begin{exercise}
    Considere a linguagem da Teoria dos Grupos do Exemplo~\ref{ex:exemplosLexicos}.
