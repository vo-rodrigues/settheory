\section{Tautologias e valorações de verdade}
...
\begin{definition}[Fórmulas proposicionais]
    ...
    Uma fórmula proposicional é expressão bem formada neste léxico.
\end{definition}
...
\begin{metadefinition}[Valoração]
...
\begin{metalemma}\label{metalemma:valoracao}
...
\begin{proof}
    ...
    A redação completa do argumento é deixada como exercício (ver Exercício~\ref{ex:metalemmaValoracao}).
\end{proof}
...
\begin{proposition}
    ...
    \begin{itemize}
        ...
        \item Se $\theta$ é $\square \psi \eta$, em que $\square$ é $\vee$, $\wedge$, $\rightarrow$ ou $\leftrightarrow$, então $\subst(\theta, \rho)$ é $\subst(\psi, \rho) \square \subst(\eta, \rho)$.
    \end{itemize}
\end{proposition}
...
\begin{exercise}\label{ex:metalemmaValoracao}
    Prove o Metalema~\ref{metalemma:valoracao}.
\end{exercise}
