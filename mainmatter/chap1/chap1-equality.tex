...
\begin{metalemma}[Congruência com termos]\label{mlem:congruenciaComTermos}
    ...
    \begin{enumerate}[label=(\roman*)]
        \item Se $f$ é um símbolo funcional $n$-ário do léxico $\mathcal L$, com $n>0$, e se $s_1,\dots,s_n,t_1,\dots,t_n$ são termos da linguagem de $T$, então
        \[
            T\vdash
        ...
        \item Se $P$ é um símbolo de predicado $n$-ário do léxico $\mathcal L$, com $n>0$, ou se $P$ é o símbolo lógico de igualdade, e se $s_1,\dots,s_n,t_1,\dots,t_n$ são termos da linguagem de $T$, então
        \[
            T\vdash
            \left(\bigwedge_{i=1}^n s_i=t_i\right)
            \rightarrow
...
\begin{proof}
    Escolha variáveis $u_1,\dots,u_n,v_1,\dots,v_n$, distintas entre si, que não ocorram em nenhum dos termos $s_1,\dots,s_n,t_1,\dots,t_n$.
    Para o item (i), aplicamos (FO17) às variáveis $u_i$ e $v_i$:
    \[
        T\vdash
        \left(\bigwedge_{i=1}^n u_i=v_i\right)
        \rightarrow
        f(u_1,\dots,u_n)=f(v_1,\dots,v_n).
    \]
    Pelo Metalema~\ref{mlem:generalizacaoDeTeoremas}, podemos tomar o fecho universal dessa fórmula nas variáveis $u_i$ e $v_i$.
    Como essas variáveis não ocorrem nos termos indicados, as instanciações universais por $s_i$ e $t_i$ são válidas, e obtemos a conclusão do item (i).

    O item (ii) é análogo, usando (FO18) no lugar de (FO17).
    Quando $P$ é o símbolo lógico de igualdade, usamos (FO18) no caso particular em que esse símbolo é tomado como predicado binário.
\end{proof}
...
\begin{proof}
    O item (1) é obtido escolhendo uma variável $u$ que não ocorra em $r$, generalizando a instância $u=u$ de (FO16) e instanciando o resultado universal pelo termo $r$.
...
\begin{proof}
    Se $\alpha$ é uma fórmula atômica dada por um símbolo de predicado $0$-ário, então $\alpha[x/s]$ e $\alpha[x/t]$ são ambas $\alpha$.
    Logo,
    \[
...
\begin{proof}
    Tratemos primeiro o caso $Q=\forall$.
    Como $y$ não ocorre livre em $\theta$, a regra de introdução do quantificador universal dá
    \[
        T\vdash \theta\rightarrow\forall y\,\theta.
    \]
    Pela preservação do quantificador universal via implicação, aplicada à hipótese
    \[
        T\vdash \theta\rightarrow(\alpha\leftrightarrow\beta),
    \]
    obtemos
    \[
        T\vdash
        \forall y\,\theta\rightarrow \forall y(\alpha\leftrightarrow\beta).
    \]
    Pela transitividade de $\rightarrow$,
    \[
        T\vdash \theta\rightarrow \forall y(\alpha\leftrightarrow\beta).
    \]

    Das regras para quantificadores, temos
    \[
        T\vdash
        \forall y(\alpha\leftrightarrow\beta)
        \rightarrow
        (\forall y\,\alpha\leftrightarrow\forall y\,\beta).
    \]
    Logo,
    \[
    ...
    Para $Q=\exists$, a hipótese implica, por raciocínio tautológico,
    \[
        T\vdash \theta\rightarrow(\neg\alpha\leftrightarrow\neg\beta).
    \]
    Pela parte universal já provada,
    \[
        T\vdash
        \theta\rightarrow(\forall y\,\neg\alpha\leftrightarrow \forall y\,\neg\beta).
    \]
    Novamente por raciocínio tautológico,
    \[
        T\vdash
        \theta\rightarrow(\neg\forall y\,\neg\alpha\leftrightarrow \neg\forall y\,\neg\beta).
    \]

    Pelas leis de negação de quantificadores e pela eliminação de dupla negação,
    \[
        T\vdash \neg\forall y\,\neg\alpha\leftrightarrow \exists y\,\alpha
        \qquad\text{e}\qquad
        T\vdash \neg\forall y\,\neg\beta\leftrightarrow \exists y\,\beta.
    \]
    Portanto,
    \[
        T\vdash \theta\rightarrow(\exists y\,\alpha\leftrightarrow \exists y\,\beta).
    \]
\end{proof}
...
\begin{proof}
    ...
    Se $\varphi$ é $\neg\psi$, então $x$ é substituível por $s$ e por $t$ em $\psi$.
    Por hipótese de indução,
    \[
        T\vdash s=t\rightarrow(\psi[x/s]\leftrightarrow\psi[x/t]).
    \]
    
    Ademais, por raciocínio tautológico,
    \[
        T\vdash
        (\psi[x/s]\leftrightarrow\psi[x/t])
        \rightarrow
    ...
    Se $\varphi$ é $\psi\square\chi$, em que $\square$ é um conectivo binário, então $x$ é substituível por $s$ e por $t$ em $\psi$ e em $\chi$.
    Pelas hipóteses de indução aplicadas a $\psi$ e a $\chi$,
    \[
        T\vdash s=t\rightarrow(\psi[x/s]\leftrightarrow\psi[x/t])
    \]
    e
    ...
    \[
        T\vdash
        ((\psi[x/s]\leftrightarrow\psi[x/t])\wedge(\chi[x/s]\leftrightarrow\chi[x/t]))
        \rightarrow
        ((\psi[x/s]\square\chi[x/s])\leftrightarrow(\psi[x/t]\square\chi[x/t])).
    \]
    Assim, 
    \[
        T\vdash
    ...
    Se $y=x$, então as substituições de $x$ por $s$ e por $t$ não atravessam o quantificador inicial.
    Assim,
    \[
    ...
    \[
        \psi[x/s]=\psi=\psi[x/t],
    \]
    e, portanto, $\varphi[x/s]=\varphi=\varphi[x/t]$.
    Nesse caso, a conclusão segue de
    \[
        T\vdash s=t\rightarrow(\varphi\leftrightarrow\varphi).
    \]
    ...
    \]
    Como $y$ não ocorre em $s$ nem em $t$, os lados dessa equivalência são exatamente $\varphi[x/s]$ e $\varphi[x/t]$.
\end{proof}
