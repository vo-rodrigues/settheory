...
Para decidir se uma expressão do tipo $A=B$ é válida, é necessário verificar previamente se $A$ e $B$ são termos bem formados, como $5$ ou $2+2$.
...
Um exemplo é o produto vetorial misto, de aridade $3$: dados $u,v,w\in\mathbb R^3$, escreve-se $[u,v,w]$ para denotar $u\cdot(v\times w)$.
...
Um \emph{léxico}\index{léxico} para uma notação polonesa é uma coleção de símbolos $\mathcal L$ tal que, para cada símbolo $s \in \mathcal L$, é atribuído um número inteiro não negativo $n_s$, chamado de \emph{aridade} de $s$.
\end{metadefinition}
...
Os famosos Axiomas de Peano, por exemplo, podem ser tomados como metateoria.
...
\begin{enumerate}[label=(\roman*)]
    \item Se $s$ em $\mathcal L$ é um símbolo de aridade $0$, então $s$ é uma expressão.
    \item Se $s$ em $\mathcal L$ é um símbolo de aridade $n$ maior do que $0$ e $e_1,\dots,e_n$ são expressões bem formadas, então a sequência $se_1\dots e_n$, obtida pela concatenação de $s$, $e_1,\dots,e_n$, é uma expressão bem formada.
...
\begin{proof}
    ...
    \begin{proof}[Prova da afirmação]
        ...
        Se $i+1=k$, segue que $S$ é $a_0e_0\dots e_i$, que é um segmento inicial próprio de $a_0e_0'\dots e_{l-1}'$, que é um segmento inicial de $S$.
        Dessa forma, $S$ é um segmento inicial próprio de $S$, o que é absurdo.
        Por isso, $i+1<k$.

        Agora, $a_0e_0\dots e_{i-1}$ e $a_0e_0'\dots e_{i-1}'$ são a mesma sequência de símbolos, de modo que $e_i$ e $e_i'$ são subexpressões de $S$ iniciadas na mesma posição.
        Dessa forma, uma deve ser segmento inicial da outra, de modo que, por (i), são iguais.
    \end{proof}
...
\begin{proof}
    ...
    A unicidade é imediata de (i) da proposição anterior: caso houvesse duas subexpressões iniciadas por $a_i$, uma delas seria um segmento inicial da outra, o que é impossível por (i) da proposição anterior.
