...
Dando um passo adicional, a sequência $2+2 = 5$~$3$ parece composta dos termos $2+2$, $5$ e $3$, mas não é bem formada, pois o símbolo $=$ --- que, intuitivamente, compara dois objetos --- está relacionando $2+2$ e $5$, deixando o símbolo $3$ sem função sintática.
...
Considere, por exemplo, a operação que recebe um vetor $v$ e índices positivos $m,n$ com $m\leq n$, retornando o subvetor que vai do $m$-ésimo ao $n$-ésimo elemento.
Em termos formais, dados $(v_1,\dots,v_k)$, $m$ e $n$ com $1\leq m\leq n\leq k$, obtém-se $(v_m,\dots,v_n)$.
Tal operação é comum em programação; em muitas linguagens, operações de fatiamento de listas ou vetores têm esse papel.
Em PHP, por exemplo, a função \texttt{array\_slice} possui esse comportamento.

A noção de aridade não se aplica apenas a símbolos que formam termos.
...
Existem também símbolos relacionais de aridade diferente de $2$.
...
Em suma, uma metateoria deve ser capaz de lidar com uma quantidade potencialmente infinita de símbolos (embora apenas finitamente muitos apareçam em cada expressão ou prova), reconhecer e comparar sequências desses símbolos e realizar raciocínios indutivos elementares, formalizando operações sobre sequências finitas (que podem ser codificadas por números naturais).

A aritmética de Peano, por exemplo, pode ser tomada como metateoria.
...
Adotaremos também a postura de que a metateoria é finitária: trata de objetos e métodos finitos, ``concretos'', que podem ser escritos e verificados.
Ao falarmos de léxicos e expressões, estamos tratando de manipulações concretas de cadeias de símbolos.
Ao falarmos de léxicos e expressões, estamos tratando de manipulações de cadeias finitas de símbolos. 
Embora a linguagem possa admitir infinitos símbolos disponíveis, cada fórmula e cada demonstração formal é finita. 
Assim, o que será usado efetivamente são procedimentos finitos de reconhecimento, comparação, substituição e concatenação de sequências de símbolos.

Ressaltamos, ainda, que a fixação de uma metateoria específica não resolve o problema filosófico sobre a fundamentação da matemática, ou a natureza da matemática, uma vez que, por ser também uma teoria, a discussão sobre ela do ponto de vista do observador externo cairia no mesmo problema que estamos descrevendo.
...
\begin{itemize}
    ...
    \item $+2{+}1$ não é uma expressão bem formada.
    Novamente, ela não é uma expressão bem formada pela regra (i), pois não consiste em um único símbolo de aridade $0$.
    Caso fosse uma expressão bem formada pela regra (ii), sendo o símbolo $+$ o primeiro símbolo da expressão, ela deveria ser, portanto, da forma $+e_1e_2$, em que $e_1$ e $e_2$ são expressões bem formadas.
    O símbolo que segue $+$ é $2$ e deve iniciar a expressão $e_1$.
    A única chance do símbolo $2$ iniciar $e_1$ é se $e_1$ for $2$, o que nos levaria a concluir que $e_2$ deveria ser $+1$, que não é uma expressão bem formada.
\end{itemize}
\end{example}

Voltemo-nos agora para uma importante vantagem teórica da notação polonesa: a eliminação da necessidade do uso de parênteses.
Examinemos, na notação usual, a seguinte expressão: $2/3\ast 4$.
O símbolo $3$ deve ser interpretado como o segundo operando de $/$ ou como o primeiro operando de $\ast$?
A resposta é que a expressão é, a princípio, ambígua, e, portanto, requer o uso de parênteses, como em $(2/3)\ast 4$ ou $2/(3\ast 4)$, ou a adoção de uma convenção de precedência.

As metaproposições restantes nesta subseção demonstrarão que, na notação polonesa, não existe tal ambiguidade.
...
Isso abre margem para argumentos indutivos: para provar que uma propriedade sobre expressões vale para toda expressão, basta provar que, caso ela valha para toda subexpressão própria de uma expressão dada, também valha para a expressão dada.
...
\begin{enumerate}[label=(\roman*)]
    ...
    \item Existe um único número $k$ e únicas expressões bem formadas $e_0,\dots,e_{k-1}$ tais que $a_0\dots a_n$ é $a_0e_0\dots e_{k-1}$ (se $k=0$, entende-se que $a_0e_0\dots e_{k-1}$ é apenas $a_0$).
...
\begin{proof}
    ...
    Se $n=0$, nossa expressão bem formada é $a_0$.
...
\begin{claim}\label{clm:comparacaoDecomposicoes} Suponha que:
        \begin{itemize}
            ...
            \item $a_0e_0'\dots e_{l-1}'$ é um segmento inicial de $S$ com $e_0',\dots,e_{l-1}'$ expressões bem formadas,
        \end{itemize}
    ...
    \begin{proof}[Prova da afirmação]
        ...
        Se $l=0$, então $l\leq k$ e não há igualdades a verificar.
        Suponhamos, então, $l>0$.
        ...
        Desse modo, devemos ter que $e_0$ é segmento inicial de $e_0'$, ou que $e_0'$ é segmento inicial de $e_0$.
        ...
        Isso conclui a indução sobre $i$.
        Como o resultado vale em particular para $i=l-1$, obtemos $l\leq k$.
    \end{proof}

    Com a afirmação, facilmente completamos a prova de (i) e (ii).
    Para (ii), basta notar que, se escrevemos $S$ como $a_0e_0\dots e_{k-1}$ e também como $a_0e_0'\dots e_{l-1}'$, ambas as sequências são segmentos iniciais de $S$, de modo que, pela afirmação aplicada duas vezes, $k=l$ e $e_i=e_i'$ para cada $i<k$.
    A aridade deve ser $k$: se $r$ é a aridade de $a_0$, então, por definição de expressão bem formada, $S$ é $a_0e_0'\dots e_{r-1}'$ para algumas subexpressões bem formadas $e_0',\dots,e_{r-1}'$; comparando essa decomposição com $a_0e_0\dots e_{k-1}$ pela afirmação, obtemos $r=k$.
...
\begin{proof}
    ...
    Se $n=0$, temos que $a_0\dots a_n$ é $a_0$, e a prova é imediata.

    Agora, se $n>0$, chamando $a_0\dots a_n$ de $S$, escrevamos $S$ como $a_0e_0\dots e_{k-1}$.

    Se $i=0$, existe uma subexpressão iniciada por $a_0$, que é a própria expressão $S$.
    Se $i>0$, então $a_i$ ocorre em $e_j$ para algum $j<k$, e, por hipótese de indução, ela inicia uma única subexpressão de $e_j$, que é, também, uma subexpressão de $a_0\dots a_n$.
\end{proof}
...
Dado um léxico $\mathcal L$, $n\geq 0$ e uma expressão bem formada $a_0\dots a_n$, para cada $i\leq n$, a única subexpressão $a_i\dots a_j$ iniciada por $a_i$ é chamada de \emph{escopo}\index{escopo} (da ocorrência do) símbolo $a_i$ em $a_0\dots a_n$.
\end{metadefinition}
