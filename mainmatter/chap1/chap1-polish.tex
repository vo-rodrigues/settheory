...
\begin{enumerate}[label=(\roman*)]
    \item Se $s\in\mathcal L$ é um símbolo de aridade $0$, então $s$ é uma expressão.
    \item Se $s\in\mathcal L$ é um símbolo de aridade $n$ maior do que $0$ e $e_1,\dots,e_n$ são expressões bem formadas, então a sequência $se_1\dots e_n$, obtida pela concatenação de $s$, $e_1,\dots,e_n$, é uma expressão bem formada.
