...
\begin{metadefinition}\index{axioma lógico}
    Os axiomas lógicos adotados neste texto consistem em fechos universais dos seguintes esquemas de fórmulas, em que $\phi$ e $\psi$ são fórmulas arbitrárias e $x$, $x_1$, $\dots$, $x_n$, $y$, $y_1, \dots$, $y_n$, $z$ são variáveis arbitrárias, e $t$ um termo.
    \begin{enumerate}
        \item Instâncias de tautologia.
...
Ressalta-se que a metadefinição acima nos dá um esquema de axiomas.
Não é um número finito de axiomas, e sim um número finito de esquemas.
Cada item gera potencialmente infinitas sentenças, e cada uma delas é um axioma.

A seguir, definiremos a noção de teoria, de demonstração, e de teorema.

\begin{metadefinition}[Teoria]\index{teoria}\index{axioma não lógico}
...
\begin{metadefinition}[Demonstração]\index{demonstração formal}\index{teorema}\index{modus ponens}
...
\begin{metadefinition}\index{inconsistência}
    Seja $T$ uma teoria de primeira ordem.
    Dizemos que $T$ é \emph{inconsistente} se existe uma sentença $\phi$ tal que $T \vdash \phi$ e $T \vdash \neg \phi$.
\end{metadefinition}

\begin{metaproposition}[Princípio da explosão]\index{princípio da explosão}
    Seja $T$ uma teoria de primeira ordem.
    Se $T$ é inconsistente, então para cada sentença $\psi$, $T \vdash \psi$.
\end{metaproposition}
\begin{proof}
    Tome $\psi$ uma sentença arbitrária.
...
\begin{metaproposition}\label{metaproposition:nonempty}
