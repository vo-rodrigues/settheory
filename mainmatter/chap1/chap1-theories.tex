...
De forma similar, podemos transportar teoremas de $T_P$ para teorias de primeira ordem arbitrárias, desde que as letras proposicionais sejam substituídas por sentenças.

\begin{metatheorem}\label{mthm:substituicaoTeoremasTP}
    Seja $\mathcal L$ um léxico de primeira ordem.
    Seja $\theta$ uma $\mathcal L_P$-fórmula sem igualdade, e seja $\rho$ uma atribuição para $\theta$ tal que, para cada letra proposicional $P$ que ocorre em $\theta$, $\rho(P)$ é uma $\mathcal L$-sentença.
    Suponha que $\subst(\theta,\rho)$ é $\phi$.

    Se $T_P\vdash \theta$, então, para toda teoria de primeira ordem $T$ de léxico $\mathcal L$,
    \[
        T\vdash \phi.
    \]
\end{metatheorem}

Para prová-lo, precisaremos do seguinte.

\begin{metalemma}\label{mlem:substituicaoProposicionalEmTeoremas}
    Seja $\mathcal L$ um léxico de primeira ordem e $\psi$ uma $\mathcal L_P$-fórmula.
    Suponha que $\rho$ é uma atribuição para $\psi$ tal que, para cada letra proposicional $P$ que ocorre em $\psi$, $\rho(P)$ é uma $\mathcal L$-sentença.

    Então:
    \begin{enumerate}[label=(\roman*)]
        \item se $x$ não ocorre livre em $\psi$, então $x$ não ocorre livre em $\subst(\psi,\rho)$;

        \item se $x$ e $y$ são variáveis e $x$ é substituível por $y$ em $\psi$, então $x$ é substituível por $y$ em $\subst(\psi,\rho)$ e
        \[
            \subst(\psi[x/y],\rho)=\subst(\psi,\rho)[x/y].
        \]
    \end{enumerate}
\end{metalemma}

\begin{proof}
    Procedemos por indução na complexidade de $\psi$.

    Se $\psi$ é uma letra proposicional $P$, então $\subst(\psi,\rho)$ é a sentença $\rho(P)$.
    Logo, nenhuma variável ocorre livre em $\subst(\psi,\rho)$.
    Além disso, $\psi[x/y]$ é $\psi$, e, como $\rho(P)$ é uma sentença,
    \[
        \subst(\psi[x/y],\rho)=\rho(P)=\rho(P)[x/y]=\subst(\psi,\rho)[x/y].
    \]

    Se $\psi$ é $u=v$, em que $u$ e $v$ são variáveis, então $\subst(\psi,\rho)$ é $u=v$.
    Nesse caso, as duas afirmações seguem diretamente da definição de substituição em fórmulas.

    O caso em que $\psi$ é $\neg\chi$ segue imediatamente da hipótese indutiva aplicada a $\chi$.

    Similarmente, o caso em que $\psi$ é $\chi\square\eta$, com $\square$ igual a $\vee$, $\wedge$, $\rightarrow$ ou $\leftrightarrow$, segue imediatamente da hipótese indutiva aplicada a $\chi$ e a $\eta$.

    Resta tratar o caso em que $\psi$ é $Qz\chi$, em que $Q$ é $\forall$ ou $\exists$.
    Primeiro, suponha que $x$ não ocorre livre em $Qz\chi$.
    Se $z=x$, então $x$ não ocorre livre em $Qx\subst(\chi,\rho)$.
    Se $z\neq x$, então $x$ não ocorre livre em $\chi$, e, pela hipótese indutiva, $x$ não ocorre livre em $\subst(\chi,\rho)$.
    Logo, em ambos os casos, $x$ não ocorre livre em $\subst(Qz\chi,\rho)$.

    Agora suponha que $x$ é substituível por $y$ em $Qz\chi$.
    Se $z=x$, então
    \[
        \subst((Qx\chi)[x/y],\rho)=\subst(Qx\chi,\rho)=Qx\subst(\chi,\rho),
    \]
    e também
    \[
        \subst(Qx\chi,\rho)[x/y]=(Qx\subst(\chi,\rho))[x/y]=Qx\subst(\chi,\rho).
    \]
    Além disso, $x$ é substituível por $y$ em $Qx\subst(\chi,\rho)$, pois não há ocorrências livres de $x$ nessa fórmula.

    Suponha, então, que $z\neq x$.
    Pelo Metalema~\ref{mlem:criteriosSubstituibilidade}, $x$ é substituível por $y$ em $\chi$ e, além disso, $x$ não ocorre livre em $\chi$ ou $z\neq y$.
    Pela hipótese indutiva, $x$ é substituível por $y$ em $\subst(\chi,\rho)$ e
    \[
        \subst(\chi[x/y],\rho)=\subst(\chi,\rho)[x/y].
    \]

    Se $x$ não ocorre livre em $\chi$, então, pelo item (i), $x$ não ocorre livre em $\subst(\chi,\rho)$.
    Se $z\neq y$, então $z$ não ocorre no termo $y$.
    Em ambos os casos, pelo Metalema~\ref{mlem:criteriosSubstituibilidade}, $x$ é substituível por $y$ em $Qz\subst(\chi,\rho)$.

    Por fim,
    \[
        \subst((Qz\chi)[x/y],\rho)
        =
        \subst(Qz(\chi[x/y]),\rho)
        =
        Qz\subst(\chi[x/y],\rho),
    \]
    que, pela hipótese indutiva, é
    \[
        Qz(\subst(\chi,\rho)[x/y])
        =
        (Qz\subst(\chi,\rho))[x/y]
        =
        \subst(Qz\chi,\rho)[x/y].
    \]
\end{proof}

\begin{proof}[Demonstração do Metateorema~\ref{mthm:substituicaoTeoremasTP}]
    Seja $\theta_0,\dots,\theta_n$ uma demonstração de $\theta$ a partir de $T_P$.

    Como antes, estendendo $\rho$ se necessário, podemos supor que $\rho$ é uma atribuição para cada $\theta_i$ e que, para cada letra proposicional $P$ que ocorre em alguma das fórmulas $\theta_0,\dots,\theta_n$, $\rho(P)$ é uma $\mathcal L$-sentença.
    Para isso, basta atribuir a cada nova letra proposicional uma sentença fixa, por exemplo $\forall z(z=z)$.

    Para cada $i\leq n$, escreva
    \[
        \phi_i=\subst(\theta_i,\rho).
    \]
    Provaremos, por indução em $i\leq n$, que $T\vdash \phi_i$.

    Suponha que $T\vdash \phi_j$ para todo $j<i$.
    Veremos que $T\vdash \phi_i$.

    Se $\theta_i$ é um axioma lógico do tipo (FO1)--(FO13), então $\phi_i$ é um axioma lógico do mesmo tipo na linguagem $\mathcal L$.
    Logo, $T\vdash \phi_i$.

    Se $\theta_i$ é um axioma do tipo (FO14), então existem uma $\mathcal L_P$-fórmula $\psi$ e variáveis $x$ e $y$ tais que $\theta_i$ é
    \[
        \forall x\psi\rightarrow \psi[x/y],
    \]
    em que $x$ é substituível por $y$ em $\psi$.
    Pelo Metalema~\ref{mlem:substituicaoProposicionalEmTeoremas}, $x$ é substituível por $y$ em $\subst(\psi,\rho)$ e
    \[
        \subst(\psi[x/y],\rho)=\subst(\psi,\rho)[x/y].
    \]
    Portanto, $\phi_i$ é
    \[
        \forall x\subst(\psi,\rho)\rightarrow \subst(\psi,\rho)[x/y],
    \]
    que é um axioma lógico do tipo (FO14).
    Logo, $T\vdash \phi_i$.

    Se $\theta_i$ é um axioma do tipo (FO15), então existem uma $\mathcal L_P$-fórmula $\psi$ e variáveis $x$ e $y$ tais que $\theta_i$ é
    \[
        \psi[x/y]\rightarrow \exists x\psi,
    \]
    em que $x$ é substituível por $y$ em $\psi$.
    Pelo Metalema~\ref{mlem:substituicaoProposicionalEmTeoremas}, $\phi_i$ é
    \[
        \subst(\psi,\rho)[x/y]\rightarrow \exists x\subst(\psi,\rho),
    \]
    que é um axioma lógico do tipo (FO15).
    Logo, $T\vdash \phi_i$.

    Se $\theta_i$ é um axioma do tipo (FO16), então $\phi_i$ é o mesmo axioma $x=x$.
    Logo, $T\vdash \phi_i$.

    Não há axiomas do tipo (FO17) em $\mathcal L_P$, pois $\mathcal L_P$ não possui símbolos funcionais de aridade positiva.
    Quanto aos axiomas do tipo (FO18), a única possibilidade em $\mathcal L_P$ é o caso em que o símbolo de predicado considerado é o símbolo lógico de igualdade.
    Nesse caso, $\phi_i$ é novamente o mesmo axioma lógico do tipo (FO18), agora visto como fórmula da linguagem $\mathcal L$.
    Logo, $T\vdash \phi_i$.

    Como $T_P$ não possui axiomas não lógicos, não há caso de axioma de $T_P$ a considerar.

    Se $\theta_i$ é obtida por \emph{modus ponens} a partir de fórmulas anteriores, então existem $j,k<i$ tais que $\theta_j$ é $\theta_k\rightarrow \theta_i$.
    Pela hipótese indutiva, $T\vdash \phi_k$ e $T\vdash \phi_j$.
    Como $\phi_j$ é $\phi_k\rightarrow \phi_i$, concatenando as demonstrações de $\phi_k$ e de $\phi_k\rightarrow\phi_i$ e aplicando \emph{modus ponens}, obtemos $T\vdash \phi_i$.

    Se $\theta_i$ é obtida pela regra de introdução do quantificador universal, então existem $\mathcal L_P$-fórmulas $\chi$ e $\eta$, uma variável $x$ e $j<i$ tais que $\theta_j$ é $\chi\rightarrow\eta$, $\theta_i$ é $\chi\rightarrow \forall x\eta$, e $x$ não ocorre livre em $\chi$.
    Pela hipótese indutiva,
    \[
        T\vdash \subst(\chi,\rho)\rightarrow\subst(\eta,\rho).
    \]
    Pelo Metalema~\ref{mlem:substituicaoProposicionalEmTeoremas}, $x$ não ocorre livre em $\subst(\chi,\rho)$.
    Aplicando a regra de introdução de quantificador universal, obtemos
    \[
        T\vdash \subst(\chi,\rho)\rightarrow \forall x\subst(\eta,\rho),
    \]
    isto é, $T\vdash \phi_i$.

    Finalmente, se $\theta_i$ é obtida pela regra de introdução do quantificador existencial, então existem $\mathcal L_P$-fórmulas $\chi$ e $\eta$, uma variável $x$ e $j<i$ tais que $\theta_j$ é $\chi\rightarrow\eta$, $\theta_i$ é $\exists x\chi\rightarrow \eta$, e $x$ não ocorre livre em $\eta$.
    Pela hipótese indutiva,
    \[
        T\vdash \subst(\chi,\rho)\rightarrow\subst(\eta,\rho).
    \]
    Pelo Metalema~\ref{mlem:substituicaoProposicionalEmTeoremas}, $x$ não ocorre livre em $\subst(\eta,\rho)$.
    Aplicando a regra de introdução de quantificador existencial, obtemos
    \[
        T\vdash \exists x\subst(\chi,\rho)\rightarrow \subst(\eta,\rho),
    \]
    isto é, $T\vdash \phi_i$.

    Portanto, $T\vdash \phi_i$ para todo $i\leq n$.
    Como $\theta_n$ é $\theta$, temos $\phi_n=\subst(\theta,\rho)=\phi$.
    Logo, $T\vdash \phi$.
\end{proof}
...
\begin{metaproposition}\label{mprop:extensaoDeTeoria}
    Sejam $T=(\mathcal L,\Sigma)$ e $T'=(\mathcal L',\Sigma')$ teorias de primeira ordem tais que todo símbolo de $\mathcal L$ é um símbolo de $\mathcal L'$ (com mesmo tipo e aridade).
