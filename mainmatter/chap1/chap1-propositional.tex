...
Nosso objetivo nesta seção é fixar três ideias básicas: a definição de fórmula proposicional, a introdução da noção de dedução formal e, por fim, a maneira como essa noção sintática de dedução se relaciona com a noção semântica de tautologia, que também será introduzida nesta seção.
...
De forma geral, uma dedução formal é uma sequência de fórmulas bem formadas de modo que cada fórmula da sequência ou é um axioma do sistema ou é obtida a partir de uma regra de inferência aplicada a fórmulas anteriores.
...
\begin{metadefinition}[Axiomas da lógica proposicional]\label{mdef:axiomasLogicaProposicional}\index{axioma!lógica proposicional}
    Os axiomas da Lógica Proposicional Clássica são as fórmulas que podem ser escritas de alguma das seguintes formas, em que $\phi$, $\psi$ e $\chi$ são fórmulas proposicionais.
...
\begin{metadefinition}[Deduções formais na lógica proposicional]\label{mdef:deducoesFormaisLogicaProposicional}\index{dedução formal!lógica proposicional}
    ...
    Dizemos que uma fórmula \emph{$\psi$ é deduzida a partir de $\Gamma$}, ou que \emph{$\psi$ é uma consequência dedutiva de $\Gamma$}, ou ainda que \emph{$\psi$ é um teorema de $\Gamma$}, e escrevemos $\Gamma \vdash_P \psi$, se existe uma dedução formal $\phi_0, \dots, \phi_n$ a partir de $\Gamma$ tal que $\phi_n$ é $\psi$.

    Quando, na notação do parágrafo anterior, $\Gamma$ é a coleção vazia de fórmulas proposicionais, dizemos que \emph{$\psi$ é um teorema da lógica proposicional}, e escrevemos $\vdash_P \psi$.
\end{metadefinition}
...
\begin{proof}
    ...
    Temos também, pela Metaproposição~\ref{metaprop:PCreflexividade} e pela monotonicidade, que:
    \[
        \{\phi\} \vdash_P \neg\phi\rightarrow \neg\phi.
    \]
    Por PC3, temos que:
...
Como aplicação, obtemos uma prova curta do Princípio da explosão.
...
\begin{proof}
    ...
    Por PC1 e modus ponens a partir de $\phi$,
    \[
        \{\phi, \neg \phi\}\vdash_P \neg\psi\rightarrow\phi.
    \]
    Aplicando o item (c) da Metaproposição~\ref{metaprop:modusTollensProposicional} à implicação $\neg\psi\rightarrow\phi$, obtemos
    \[
        \{\phi, \neg \phi\}\vdash_P \neg\phi\rightarrow\neg\neg\psi.
    \]
    Como $\{\phi, \neg \phi\}\vdash_P \neg\phi$, segue por modus ponens que $\{\phi, \neg \phi\}\vdash_P \neg\neg\psi$.
...
$T$ e $F$ são símbolos arbitrários que não fazem parte da linguagem da lógica proposicional, fixados para denotar intuitivamente os valores de verdade verdadeiro e falso, respectivamente.
...
\begin{metadefinition}\label{mdef:valorVerdadeFormulaProposicional}Seja $v$ uma valoração de variáveis proposicionais.
    ...
    Se $v$ não é uma valoração de $\phi$, então $v(\phi)$ é $I$.
...
Uma primeira observação é que valorações de $\phi$ que coincidem em todas as variáveis proposicionais que ocorrem em $\phi$ atribuem o mesmo valor de verdade a $\phi$.
Desse modo, valores eventualmente atribuídos a variáveis proposicionais que não ocorrem em $\phi$ não afetam o valor de verdade de $\phi$.
...
\begin{proof}
    ...
    Logo, $v(\phi)$ é $\neg v(\psi)$, e $v'(\phi)$ é $\neg v'(\psi)$, de modo que $v(\phi)$ é igual a $v'(\phi)$, e não é $I$.
    ...
    Logo, $v(\phi)$ é $v(\psi) \square v(\eta)$, e $v'(\phi)$ é $v'(\psi) \square v'(\eta)$, de modo que $v(\phi)$ é igual a $v'(\phi)$, e não é $I$.
\end{proof}
...
\begin{proof}
    ...
    Desse modo, $v(\subst(\xi, \rho))$ é $v(\subst(\psi, \rho) \square \subst(\eta, \rho))$, que é $v(\subst(\psi, \rho)) \square v(\subst(\eta, \rho))$, que é $\hat v(\psi) \square \hat v(\eta)$, que é $\hat v(\xi)$.
\end{proof}
...
\begin{proof}
    ...
    Pelo corolário anterior, é suficiente mostrar que cada esquema de axiomas da Metadefinição~\ref{mdef:axiomasLogicaProposicional} é uma tautologia quando $\psi$, $\phi$ e $\chi$ são variáveis proposicionais distintas, digamos, $P$, $Q$ e $R$.
    ...
    Desse modo, $u$ é uma valoração de $\phi_k\rightarrow \phi_i$.
...
\begin{proof}
    ...
    Por PC6 e duas aplicações sucessivas de modus ponens, temos $\Gamma_v\vdash_P \psi\land\eta$.
    ...
    \]
    Pelo item (c) da Metaproposição~\ref{metaprop:modusTollensProposicional},
    \[
        \Gamma_v\vdash_P \neg\psi\rightarrow \neg(\psi\land\eta).
    \]
    Assim, por modus ponens, $\Gamma_v\vdash_P \neg(\psi\land\eta)$, e, portanto, $\Gamma_v\vdash_P \theta^v$.

    O caso em que $v(\eta)=F$ é análogo, utilizando PC5 no lugar de PC4.
    ...
    Por PC7 e modus ponens, $\Gamma_v\vdash_P \psi\lor\eta$.
    Portanto, $\Gamma_v\vdash_P \theta^v$.

    O caso em que $v(\eta)=T$ é análogo, utilizando PC8 em vez de PC7.
    ...
    Por PC1 e modus ponens, a partir de $\Gamma_v\vdash_P \neg\psi$ obtemos
    \[
        \Gamma_v\vdash_P \neg\eta\rightarrow\neg\psi.
    \]
    Aplicando o item (c) da Metaproposição~\ref{metaprop:modusTollensProposicional} à implicação $\neg\eta\rightarrow\neg\psi$, obtemos
    ...
    \]
    Como $\Gamma_v\vdash_P \eta\rightarrow\eta$ pela Metaproposição~\ref{metaprop:PCreflexividade} e pela monotonicidade, segue de PC9 e duas aplicações de modus ponens que
    \[
        \Gamma_v\vdash_P (\psi\lor\eta)\rightarrow\eta.
    \]
    Aplicando novamente o item (c) da Metaproposição~\ref{metaprop:modusTollensProposicional}, obtemos
    \[
        \Gamma_v\vdash_P \neg\eta\rightarrow\neg(\psi\lor\eta).
    \]
    Como $\Gamma_v\vdash_P \neg\eta$, segue por modus ponens que $\Gamma_v\vdash_P \neg(\psi\lor\eta)$, e, portanto, $\Gamma_v\vdash_P \theta^v$.
    ...
    Por PC1 e modus ponens, $\Gamma_v\vdash_P \neg\eta\rightarrow\neg\psi$.
    Logo, pelo item (c) da Metaproposição~\ref{metaprop:modusTollensProposicional}, aplicado à contrapositiva de $\psi\rightarrow\eta$, temos $\Gamma_v\vdash_P \psi\rightarrow\eta$.
    ...
    Em $\Gamma_v\cup\{\psi\rightarrow \eta\}$, temos $\psi$ por monotonicidade e $\psi\rightarrow\eta$ por hipótese.
    Logo, por modus ponens, $\Gamma_v\cup\{\psi\rightarrow \eta\}\vdash_P \eta$.
    Pelo Teorema da Dedução,
    \[
        \Gamma_v\vdash_P (\psi\rightarrow\eta)\rightarrow\eta.
    \]
    Pelo item (c) da Metaproposição~\ref{metaprop:modusTollensProposicional},
    \[
        \Gamma_v\vdash_P \neg\eta\rightarrow\neg(\psi\rightarrow\eta).
    ...
    Por PC1 e modus ponens, obtemos
    \[
        \Gamma_v\vdash_P \neg\eta\rightarrow\neg\psi
        \qquad\text{e}\qquad
        \Gamma_v\vdash_P \neg\psi\rightarrow\neg\eta.
    ...
    Assim, pelo Teorema da Dedução,
    \[
        \Gamma_v\vdash_P (\psi\leftrightarrow\eta)\rightarrow\eta.
    \]
    Pelo item (c) da Metaproposição~\ref{metaprop:modusTollensProposicional},
    \[
        \Gamma_v\vdash_P \neg\eta\rightarrow\neg(\psi\leftrightarrow\eta).
    ...
    O caso em que $v(\psi)=F$ e $v(\eta)=T$ é análogo, utilizando PC11 no lugar de PC10.
...
\begin{proof}
    ...
    \]
    Pelo Metalema~\ref{mlem:terceiroExcluidoProposicional} e pela monotonicidade,
    \[
        \Gamma\vdash_P \phi\vee\neg\phi.
    \]
...
\begin{proof}
    ...
    \[
        s(\phi)=T.
    \]
    Logo, pela definição de $\phi^s$, temos que $\phi^s$ é $\phi$.
    ...
    Portanto, por indução decrescente, $(\ast_0)$ é verdadeira.
...
\begin{metadefinition}
    ...
    Dizemos que $\psi$ é uma consequência tautológica de $\Gamma$, e escrevemos $\Gamma\models_P \psi$, se, e somente se, para toda valoração $v$ de $\psi$ e de cada fórmula $\xi\in \Gamma$, se $v(\xi)=T$ para cada $\xi\in \Gamma$, então $v(\psi)=T$.
\end{metadefinition}

\begin{metacorollary}\label{mcor:teoremaSemanticaProposicional}
    Seja $\Gamma$ uma coleção finita de fórmulas proposicionais, e seja $\psi$ uma fórmula proposicional.
    
    Então $\Gamma\vdash_P \psi$ se, e somente se, $\Gamma\models_P \psi$.
\end{metacorollary}
...
\begin{proof}
    ...
    Seja $v$ uma valoração de $\psi$ e de cada fórmula de $\Gamma$ tal que $v(\xi)=T$ para cada $\xi\in \Gamma$.
    ...
    Agora suponha que $\Gamma\models_P \psi$.
    Devemos ver que $\Gamma\vdash_P \psi$.
    Equivalentemente, veremos que $\Gamma'\vdash_P \phi\rightarrow \psi$.
    Por hipótese indutiva, basta ver que $\Gamma'\models_P \phi\rightarrow \psi$.
