...
\begin{proof}
    ...
    Note que $\varphi[x/y][y/x]$ é $\phi$, na primeira linha.
    Nela, $y$ é substituível por $x$ em $\varphi[x/y]$ porque $y$ não ocorre livre em $\varphi$.
\end{proof}
...
\begin{proof}
    ...
    Pelo Metalema~\ref{mlem:substituicaoInversaEmFormulas}, para cada $i\leq n$, $\theta[c/y]$ não possui o símbolo $c$, e, portanto, é uma $\mathcal L$-fórmula.
    ...
    \textbf{Caso 2.} Suponha que e $\theta_i$ é um axioma lógico de $\mathcal L(c)$. 
    ...
    \]
    que é uma instância de FO14 (substitua $y$ por $y$, por exemplo).
    ...
    Pelo Metalema~\ref{mlem:substituicaoInversaEmFormulas}, segue que $T\vdash \theta[x/y]$.
    Pelo Metalema~\ref{mlem:generalizacaoDeTeoremas},
    \[
        T\vdash \forall y\,\theta[x/y].
    \]
    Como $y$ não ocorre em $\theta$, pelo Metalema~\ref{mlem:renomeacaoVariavelUniversal}, concluímos que $T\vdash \forall x\,\theta$.
\end{proof}
