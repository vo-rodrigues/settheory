...
\begin{definition}[Substituição em termos]
    ...
    Dado um termo $s$, $s[x/t]$ é o termo obtido a partir de $s$ substituindo-se todas as ocorrências livres de $x$ por $t$.
    ...
    \begin{itemize}
        ...
        \item Se $s$ é $fe_1 \dots e_n$, em que $f$ é um símbolo funcional $n$-ário e $e_1, \dots, e_n$ são termos para os quais $s[x/t]$ já foi definido, então $s[x/t]$ é $fe_1[x/t] \dots e_n[x/t]$.
    \end{itemize}
\end{definition}
...
\begin{example}
    ...
    A variável $x$ não é substituível pelo termo $y$ nessa forma, pois $x$ está no escopo de um quantificador de uma variável que ocorre em $y$ (o próprio $y$).

    Notemos que, ao substituir $x$ por $y$ nessa fórmula, obtemos $\exists y \, (y \neq y)$, que é intuitivamente falso.
\end{example}

\begin{example}
    Considere a fórmula $\forall x \exists y \, (x \neq y)$.
