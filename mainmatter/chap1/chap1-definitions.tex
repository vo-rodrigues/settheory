...
\begin{metadefinition}[Fórmula do valor de um termo]\label{mdef:formulaDoValorDeUmTermo}\index{termo!fórmula do valor}\index{eliminação de símbolos definidos!termos}
    ...
    Definimos por recursão, escolhendo sempre variáveis auxiliares novas, uma $\mathcal L$-fórmula $\delta_t$ cujas variáveis livres estão entre as variáveis de $t$ e mais uma variável $y$, lida como
    ...
    \begin{enumerate}[label=(\roman*)]
        ...
        $y_1, \dots, y_m$ são variáveis auxiliares duas a duas distintas e distintas de $x_1,\ldots,x_n,y$,
        e as variáveis auxiliares internas das fórmulas $\delta_{t_i}$ são renomeadas, se necessário, para evitar colisões.
    \end{enumerate}
\end{metadefinition}
...
\begin{proof}
    ...
    Se $R$ tem aridade $0$, então $\psi^\ast$ é a própria fórmula $R$, e A e B são imediatas por uma instância de tautologia e, se necessário, por generalização de teoremas.
    ...
    Se $P$ tem aridade $0$, então $\psi$ é $P$, $\psi^\ast$ é $\Def_P$, e a conclusão segue do axioma definidor $P\leftrightarrow\Def_P$ e, se necessário, por generalização de teoremas.
