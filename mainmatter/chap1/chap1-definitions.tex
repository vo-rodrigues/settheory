...
\begin{proof}
    Sejam $y, z$ variáveis que não ocorrem em $\varphi$.
    ...
    Para isso, basta concluir que $T\vdash \forall z\,\forall x \bigl((\varphi \wedge \varphi[x/z])\rightarrow x=z\bigr)$.

    Fixe uma constante $c$ para $z$.
    Basta ver que na teoria expandida podemos provar $\forall x \bigl((\varphi[z/c] \wedge \varphi[x/z][z/c])\rightarrow x=c\bigr)$, ou seja, que $\forall x \bigl((\varphi \wedge \varphi[x/c])\rightarrow x=c\bigr)$.

    Portanto, podemos instanciar $y$ em $c$, obtendo $\forall x\bigl((\varphi \wedge \varphi[x/c])\rightarrow x=c\bigr)$, como desejado.
\end{proof}
