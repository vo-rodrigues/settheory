...
\begin{metadefinition}[Quantificador de existência única]\index{existência única}
...
\begin{metadefinition}[Extensão por definição]\label{def:extensao-por-definicao}\index{extensão por definição}
    ...
    \begin{itemize}
        ...
        em uma (única) definição explícita no sentido da Definição~\ref{def:definicao-explicita}.
    \end{itemize}
\end{metadefinition}
...
\begin{metadefinition}[Fórmula do valor de um termo]\label{def:delta-termo}\index{termo!fórmula do valor}\index{eliminação de símbolos definidos!termos}
...
A definição acima introduz variáveis auxiliares $y_1,\ldots,y_m$ na fórmula $\delta_t$,
...
(cf.\ Metalema~\ref{metalemma:renomeacaoUniversal} e seu análogo existencial).

\begin{metalemma}[Eliminação de termos]\label{metalemma:elim-termos}\index{eliminação de símbolos definidos!termos}
    ...
    Seja $\delta_t(x_1,\ldots,x_n,y)$ como na Definição~\ref{def:delta-termo}. Então:
...
Pela Definição~\ref{def:delta-termo}, $\delta_t(y)$ é $\Def_c(y)$.
...
Pela Definição~\ref{def:delta-termo}, $\delta_t(z,y)$ é $y=z$, isto é, $\Def_{z}(z,y)$.
...
Pela Definição~\ref{def:delta-termo},
\[
...
\begin{itemize}
    \item Se $f$ é novo, isto é exatamente a condição da Definição~\ref{def:definicao-explicita}:
...
\begin{metadefinition}[Tradução $\psi\mapsto \psi^\ast$]\label{def:traducao-formulas}\index{eliminação de símbolos definidos!fórmulas}
...
\begin{metalemma}[Eliminação de fórmulas]\label{metalemma:elim-formulas}\index{eliminação de símbolos definidos!fórmulas}
