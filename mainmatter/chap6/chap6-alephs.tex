...
Depois de definir os primeiros $\omega$ termos dessa sequência, isto é, $\omega$, $\omega^+$, $\omega^{++}$, $\ldots$, é possível definir o próximo cardinal:
\[
    \bigcup\{\omega, \omega^+, \omega^{++}, \ldots\}
\]
A formalização desse raciocínio nos leva à sequência dos alephs, que é definida da seguinte forma:
\begin{definition}\label{def:sequenciaDosAlephs}\index{aleph|seealso{sequência dos alephs}}\index{alephs|see{aleph}}
...
Se $\alpha=0$, então $\aleph_0 = \omega$ é um cardinal, e a outra parte da afirmação também é trivial.
...
Neste texto, por motivos didáticos, adotaremos a convenção de que, ao escrevermos $\omega_\alpha$, estamos pensando em $\aleph_\alpha$ como o conjunto $\aleph_\alpha$ munido de sua estrutura natural de boa-ordem, ou seja, enquanto ordinal.
