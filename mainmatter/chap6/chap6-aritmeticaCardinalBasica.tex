...
\index{aritmética cardinal}
\index{soma cardinal}
\index{produto cardinal}

É possível definir operações de aritmética cardinal para simplificar o cálculo de cardinalidades infinitas.
Tais operações, por ora apenas de soma e produto, não coincidem com a noção de aritmética ordinal.

\begin{definition}\index{soma cardinal}\index{produto cardinal}
...
\begin{example}
    ...
    O \textbf{produto ordinal} $\omega\cdot\omega$ é o ordinal $\omega \cdot \omega>\omega+\omega>\omega$, um ordinal enumerável.

    A \textbf{soma cardinal} de $\omega$ com $\omega$ é $|\{0\}\times \omega \cup \{1\}\times \omega|=|\omega|=\omega$.
...
\begin{lemma}
    Seja $A$ um conjunto e suponha que exista um conjunto bem ordenável $B$ e uma função sobrejetora $f:B\to A$.
...
\begin{proof}
    Seja $B'=B\setminus A$.
    Temos que $A$ e $B'$ são bem ordenáveis e disjuntos e, portanto, $A\cup B'=A\cup B$ é bem ordenável.
    Além disso, $|B'|\leq |B|$.
    Portanto:
    \[
...
\end{lemma}
\begin{proof}
    A prova é simples e será deixada como exercício ao leitor.
\end{proof}
