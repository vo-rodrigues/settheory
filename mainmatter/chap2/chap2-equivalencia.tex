...
\begin{definition}[Relação de equivalência]\index{relação de equivalência}
    Uma \emph{relação de equivalência} é um par $(A, \sim)$ onde $A$ é um conjunto e $\sim$ é uma relação binária em $A$ tal que para todo $x, y, z \in A$, valem:
    \begin{itemize}
...
\begin{example}\label{ex:equiv-funcao}
...
\begin{definition}[Classe de equivalência]\index{classe de equivalência}\index{conjunto quociente}
    ...
    O conjunto de todas as classes de equivalência de elementos de $A$ é denotado por $A/{\sim}$.
\end{definition}

Deixamos os seguintes fatos básicos para verificação do leitor (Exercício~\ref{ex:equiv-basicas}).

\begin{proposition}\label{prop:equiv-basicas}
    ...
    \begin{enumerate}[label=(\alph*)]
        ...
        \item $\displaystyle A=\bigcup A/{\sim}$.
    \end{enumerate}
\end{proposition}

A proposição anterior mostra que duas classes de equivalência diferentes são disjuntas, e que, reunidas, elas formam o conjunto $A$.
...
\begin{definition}[Partição]\index{partição}
    Seja $A$ um conjunto.
...
Decorre da Proposição~\ref{prop:equiv-basicas} que, dada uma relação de equivalência, o conjunto $A/{\sim}$ é uma partição de $A$.
Reciprocamente, temos a seguinte proposição.

\begin{proposition}\label{prop:particao-equivalencia}
...
\begin{proof}
    Dado $x \in A$ e seja $X \in \mathcal{E}$ tal que $x \in X$.
...
Da proposição anterior, iniciando-se com uma relação de equivalência, passando-se para a partição induzida por ela e, depois, passando-se para a relação de equivalência induzida por essa partição, chega-se à relação de equivalência inicial.
...
\begin{proof}
    ...
    \[
    x \sim_{\mathcal{E}} y \iff \exists X \in A/{\sim}\, (x \in X \wedge y \in X) \iff \exists z \in A\, (x, y \in [z]) \iff x \sim y,
    \]
\end{proof}
...
\begin{exercise}\label{ex:equiv-basicas}
    Prove a Proposição~\ref{prop:equiv-basicas}.
\end{exercise}

\begin{exercise}
    Seja $A$ um conjunto.
    Prove que $(A, =)$ e $(A, A \times A)$ são relações de equivalência.
    Determine, em cada caso, as classes de equivalência e o quociente correspondente.
\end{exercise}
...
\begin{exercise}
    ...
    Mostre que existe uma função $f$ com $\dom f=A$ tal que, $\sim$ é $\sim_f$.
\end{exercise}
