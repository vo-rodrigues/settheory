...
\begin{definition}
    ...
    \begin{enumerate}[label=(\alph*)]
        \item $f$ é uma função de $A$ em $B$, o que denotamos por $f:A\rightarrow B$, se $\dom f = A$ e $\ran f \subseteq B$;
...
\begin{lemma}
    Sejam $f:A\rightarrow B$ e $g:C\rightarrow D$ funções.
    Então $g\circ f$ é uma função, $g\circ f:f^{-1}[C] \rightarrow D$ e, para todos $a \in f^{-1}[C]$, temos que $(g\circ f)(a) = g(f(a))$.
\end{lemma}
...
\begin{definition}
    Uma função $f$ é dita ser \textbf{injetora} se, para quaisquer $a, a' \in \dom f$ e $b \in \ran f$, se $f(a) = b$ e $f(a') = b$, então $a = a'$.

    Dizemos que $f$ é \textbf{invertível} se, e somente se $f^{-1}$ é uma função.
\end{definition}

Dentro do formalismo de Teoria dos Conjuntos, tais noções são equivalentes.
