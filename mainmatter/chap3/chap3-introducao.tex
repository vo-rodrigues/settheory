\section{O conjunto dos números naturais}\label{sec:introducaoAosNaturais}
\index{equipotência}
\index{sucessor}
\index{conjunto indutivo}
...
\begin{definition}[$\BST$]\label{def:equipotencia}\index{equipotência}
    Sejam $A$, $B$ conjuntos.
...
\begin{proposition}[$\BST$]\label{prop:equipotenciaEquivalencia}
...
Uma primeira ideia para definir-se, digamos, o número $1$, seria adotar uma estratégia análoga e definir o número $1$ como a classe de equivalência de conjuntos que têm exatamente um elemento.
...
Não está imediatamente claro o que o ``e assim por diante'' significaria, mas não divagaremos sobre isso, uma vez que o Lema~\ref{lem:naoExisteConjuntoDeSingletos} nos mostra que essa estratégia é invariavelmente condenada ao fracasso.
\begin{lemma}[$\BST$]\label{lem:naoExisteConjuntoDeSingletos}
...
\begin{proof}
    ...
    Mas já foi mostrado na Proposição~\ref{prop:paradoxoDeRussell} que não existe um conjunto com essa propriedade.
    Chegamos, assim, a uma contradição.
\end{proof}

Seguimos para a estratégia seguinte.
Em vez de definirmos o número $1$ como a classe de equivalência de todos os singletos, definiremos o número $1$ como um \emph{representante} dessa classe.
...
\begin{definition}[$\BST$]\label{def:sucessor}\index{sucessor}
...
\begin{definition}[$\BST$]\label{def:conjuntoIndutivo}\index{conjunto indutivo}
...
\begin{axiom}[Axioma do Infinito]\label{ax:infinito}\index{axioma!do infinito}
...
\begin{definition}[$\BSTI$]\label{def:numerosNaturais}\index{conjunto dos números naturais}
    ...
    Um número natural é um elemento de $\omega$.
\end{definition}

Vamos mostrar que tal conjunto existe e é único.

\begin{lemma}[$\BSTI$]\label{lem:existenciaEUnicidadeDeOmega}
    Existe um único conjunto $J$ como na Definição~\ref{def:numerosNaturais}.
\end{lemma}
\begin{proof}
...
\begin{proposition}[$\BSTI$]\label{prop:omegaIndutivo}
...
\begin{proposition}[$\BSTI$]\label{prop:omegaEhUnicoSubconjuntoIndutivoDeSi}
...
\begin{proposition}[$\BSTI$](Esquema da Indução)\label{prop:esquemaDaInducaoNosNaturais}\index{indução}
...
\begin{proof}
    ...
    Como a Proposição~\ref{prop:omegaEhUnicoSubconjuntoIndutivoDeSi} diz que o único subconjunto indutivo de $\omega$ é $\omega$, segue que $I=\omega$.
    Assim, $\forall x \in \omega \, \phi(x, t_1, \dots, t_n)$.
\end{proof}

Podemos agora formalmente definir os $10$ primeiros números naturais:
\begin{definition}[$\BST$]\label{def:primeirosNaturais}
    \begin{align*}
        0 &= \emptyset,\\
        1 &= S(0) = \{0\},\\
        2 &= S(1) = \{0,1\},\\
...
Uma vez desenvolvida a aritmética básica dos números naturais, é possível formalizar o sistema decimal posicional usual a partir da concatenação destes símbolos.

Finalizaremos esta subseção com um fato básico sobre os números naturais: todo número natural ou é $0$, ou é o sucessor de algum número natural.

\begin{proposition}[$\BSTI$]\label{prop:todoNaturalEhZeroOuSucessor}
    Para todo $n \in \omega$, $n=0$ ou existe $m \in \omega$ tal que $n=S(m)$.
\end{proposition}
\begin{proof}
    Procedemos por indução em $n \in \omega$.
    
    Para $n=0$, a tese é verdadeira.
    
    Suponha que a tese seja verdadeira para um $m \in \omega$.
    Devemos ver que a tese é verdadeira para $S(m)$.
    Mas $S(m)$ é o sucessor de $m$, e $m \in \omega$ por hipótese.
    Assim, $S(m)$ é o sucessor de um número natural, e a tese é verdadeira para $S(m)$.

    Logo, por indução, a tese é verdadeira para todo $n \in \omega$.
\end{proof}
