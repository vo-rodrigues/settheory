\section{Recursão nos números naturais}\label{sec:recursaoNosNaturais}
\index{recursão}
\index{teorema!da recursão}
\index{sequência de Fibonacci}

Intuitivamente, o significado de $m+n$ consiste em tomar $m$ e aplicar a operação de sucessor $n$ vezes.
Como uma criança aprendendo a somar, parte-se de $m$ e, ``contando nos dedos'', percorrem-se $n$ números, e o resultado é $m+n$.
...
Lembramos ao leitor que construções autorreferentes, no geral, podem ser problemáticas em Teoria dos Conjuntos (ver, por exemplo, o Exercício~\ref{exr:compreensaoAutorreferente}).
...
No geral, ao se definir uma sequência $s_p=(s_p(n): n \in \omega)$, o valor $s_p(n)$ pode depender de todos os valores anteriores, $t=(s_p(k): k < n)$, e não apenas do valor imediatamente anterior, $s_p(n-1)$, bem como de parâmetros $p$ que atuem em toda a sequência, como $m$ na definição de $m+n$.
...
Ora, a priori, calculam-se iteradamente os termos anteriores até chegar em $F_p(n)$.
Mais precisamente, calculam-se $F_p(0), F_p(1), \dots, F_p(n-1)$ antes de chegar em $F_p(n)$.
...
\begin{definition}[$Z^-$]\label{def:familiasDeFuncoesParametrizadas}
    Sejam $A, B, C$ conjuntos.
    Dada uma função $f: A\times B\to C$ e $a\in A$, definimos $f_a: B\to C$ por $f_a(b)=f(a, b)$.

    Reciprocamente, dada uma família de funções $f_a: B\to C$ indexada por $a\in A$, definimos $f: A\times B\to C$ por $f(a, b)=f_a(b)$.
\end{definition}

\begin{theorem}[$Z^-$](Teorema da Recursão)\label{thm:teoremaDaRecursao}\index{teorema!da recursão}
...
\begin{proof}
    ...
    \]
    o que é possível pelo Esquema da Compreensão; escrevemos $f_p=h(p)$.
    ...
    Para a unicidade, veremos que, se $f:P\times \omega\to A$ e $g:P\times \omega\to A$ são tais que, para todo $p \in P$ e $n\in \omega$, $f_p(n)=G(p, f_p\restriction n)$ e $g_p(n)=G(p, g_p\restriction n)$, então $f=g$.
...
\section*{Exercícios}
\begin{exercise}[$Z^-$](Teorema da recursão sem parâmetros)
