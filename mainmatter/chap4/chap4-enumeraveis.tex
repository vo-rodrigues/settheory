...
O conceito de conjunto enumerável é uma generalização do conceito de conjunto finito.
Alguns conjuntos infinitos são enumeráveis.
Intuitivamente, eles são ``infinitos, mas não muito grandes'', uma vez que podemos contá-los utilizando números naturais.

\begin{definition}[$Z^--P$]\label{def:enumerabilidade}\index{enumerabilidade}
    Um conjunto $A$ é \emph{enumerável} se é finito, ou se é equipotente a $\omega$.
\end{definition}

\begin{lemma}[$Z^-$]\label{lem:subconjuntoDeOmegaEhEnumeravel}
...
\begin{proof}
    ...
    Desse modo, $\omega\preceq X$.
    Como $X\subseteq \omega$, segue do Teorema de Cantor-Schröder-Bernstein que $X\approx \omega$, e, portanto, $X$ é enumerável.
\end{proof}

\begin{proposition}[$Z^-$]\label{prop:caracterizacaoDaEnumerabilidade}
Seja $A$ um conjunto.
São equivalentes:
\begin{enumerate}[label=(\alph*)]
    \item $A$ é enumerável.
    \item Existe uma função injetora $f:A\rightarrow \omega$.
\end{enumerate}
\end{proposition}
\begin{proof}
...
\begin{proposition}[$Z^-$]\label{prop:omegaQuadradoEhEnumeravel}
