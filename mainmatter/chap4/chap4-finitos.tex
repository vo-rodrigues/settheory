...
\begin{proof}
    ...
    Considere $g:n\to X\setminus \{n\}$ dada por
     ...
     A figura abaixo ilustra o processo de construção da função $g$ a partir da função $f$ quando $k\neq n$.
     ...
     \begin{tikzpicture}[
         ...
         texto/.style={font=\small, align=center}
     ]
         \begin{scope}[xshift=0cm]
             \node[texto] at (1.8,2.2) {$f:n+1\to X$};

             \node[ponto] (fm) at (0,1.2) {$m$};
             \node[ponto, fill=gray!15] (fk) at (1.8,1.2) {$k$};
             \node[ponto, fill=blue!10] (fn) at (3.6,1.2) {$n$};

             \node[ponto] (fimgm) at (0,-0.2) {$f(m)$};
             \node[ponto, fill=gray!15] (fimgk) at (1.8,-0.2) {$n$};
             \node[ponto, fill=blue!10] (fimgn) at (3.6,-0.2) {$f(n)$};
         ...
         \draw[->, thick] (4.9,0.5) -- (6.1,0.5);

         \begin{scope}[xshift=7cm]
             \node[texto] at (1.3,2.2) {$g:n\to X\setminus\{n\}$};

             \node[ponto] (gm) at (0,1.2) {$m$};
             \node[ponto, fill=gray!15] (gk) at (2.6,1.2) {$k$};

             \node[ponto] (gimgm) at (0,-0.2) {$f(m)$};
             \node[ponto, fill=blue!10] (gimgk) at (2.6,-0.2) {$f(n)$};
     ...
     \captionof{figure}{Subcaso $k\neq n$ do Caso 2: a seta $k\mapsto n$ é substituída por $k\mapsto f(n)$; para $m<n$ com $m\neq k$, continua valendo $g(m)=f(m)$.}
     \label{fig:lemaSubconjuntoProprioNaturalFuncaoG}
     \end{center}
...
\begin{corollary}[$Z^-$]Sobre finitude, temos o seguinte.\label{cor:finitoCardinal}
    \begin{enumerate}[label=(\alph*)]
        ...
        \item $\omega$ é infinito.\label{item:omegaInfinito}
    \end{enumerate}
\end{corollary}
\begin{proof}
...
\begin{proof}
    ...
    Seja $(A, <)$ uma ordem total estrita com cardinalidade $n+1$.
...
Por hipótese indutiva, existe um isomorfismo de ordens $\phi': A'\to n$.
