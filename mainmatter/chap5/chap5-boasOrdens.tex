\section{Boas ordens}\label{sec:boasOrdens}
\index{ordem!boa}
\index{segmento inicial}
Nesta seção, nos aprofundaremos um pouco mais na noção de boa ordem.

\begin{definition}[$\BST$]\label{def:segmentoInicial}\index{segmento inicial}
...
\begin{lemma}[$\BST$]\label{lem:funcaoCrescenteAcimaDaIdentidade}
...
\begin{lemma}[$\BST$]\label{lem:isomorfismoParaSegmentoInicialEhIdentidade}
    Seja $S$ um segmento inicial de uma boa ordem $A$ e $f:A\to S$ um isomorfismo.
    Então $S=A$ e $f$ é a função identidade de $A$.
\end{lemma}
\begin{proof}
    Pelo Lema~\ref{lem:funcaoCrescenteAcimaDaIdentidade}, para todo $a\in A$, $a\leq f(a)$.
    Como $f(a)\in S$ e $S$ é um segmento inicial de $A$, segue que $a\in S$.
    Assim, $S=A$.

    Pelo Lema~\ref{lem:funcaoCrescenteAcimaDaIdentidade}, agora aplicado à $f^{-1}: A\to A$, para todo $a\in A$, $a\leq f^{-1}(a)$.
    Aplicando $f$ em ambos os lados, temos que $f(a)\leq a$.
    Assim, $a=f(a)$ para todo $a\in A$.
\end{proof}

\begin{corollary}[$\BST$]\label{cor:unicidadeDoIsomorfismoDeBoasOrdens}
...
\begin{proof}
    ...
    Pelo Lema~\ref{lem:isomorfismoParaSegmentoInicialEhIdentidade}, $g^{-1}\circ f$ é a função identidade de $A$, e, assim, $f=g$.
\end{proof}

\begin{definition}[$\BST$]\label{def:segmentoInicialDeterminadoPorElemento}
    Seja $A$ uma boa ordem e $a\in A$.
    Definimos $A[a] = \{x\in A : x<a\}$.
\end{definition}
\begin{lemma}[$\BST$]\label{lem:segmentosIniciaisPropriosSaoDeterminadosPorElementos}
    Seja $A$ uma boa ordem e $a\in A$.
...
\begin{lemma}[$\BST$]\label{lem:isomorfismoPreservaSegmentosIniciais}
...
\begin{lemma}[$\BST$]\label{lem:segmentosIniciaisDeSegmentosIniciais}
...
\begin{theorem}[$\BST$]\label{thm:comparacaoDeBoasOrdens}\index{teorema!de comparação de boas ordens}
    Sejam $A$ e $B$ boas ordens.
...
\end{theorem}
\begin{proof}
    A unicidade do isomorfismo decorre do Corolário~\ref{cor:unicidadeDoIsomorfismoDeBoasOrdens}.
    Iniciaremos mostrando que somente uma alternativa é possível.
    Se valem (a) e (b), compondo os isomorfismos, temos que $A$ é isomorfo a um segmento inicial próprio de $A$, o que é impossível pelo Lema~\ref{lem:isomorfismoParaSegmentoInicialEhIdentidade}.
    ...
    Pelo Lema~\ref{lem:isomorfismoPreservaSegmentosIniciais}, $g|A[a]: A[a]\to B[g(a)]$ é um isomorfismo.
    Assim, $(a,g(a))\in f$.
...
\begin{exercise}
    Prove o Lema~\ref{lem:segmentosIniciaisDeSegmentosIniciais}.
\end{exercise}
\begin{exercise}[$\BST$]
...
\begin{exercise}[$Z^-$]
    ...
    \]
\end{exercise}
\begin{exercise}[$\BST$]\label{exr:uniaoDeBoasOrdens}
