...
\begin{proposition}[$\ZFmPI$](Esquema de boa ordenação)\label{prop:esquemaDaBoaOrdenacao}\index{esquema de boa ordenação}
...
\begin{proposition}[$\ZFmPI$](Esquema da indução transfinita)\label{prop:esquemaDaInducaoTransfinita}
    Seja $\phi(\alpha, t_1, \dots, t_n)$ uma fórmula.
...
\begin{definition}[$\ZFmPI$]\label{def:sequenciaTransfinita}\index{sequência transfinita}
...
\begin{definition}[$\ZFmPI$]\label{def:computacaoParcialTransfinita}
...
\begin{lemma}[$\ZFmPI$]\label{lem:compatibilidadeDeComputacoesParciais}
...
\begin{corollary}[$\ZFmPI$]\label{cor:unicidadeDeComputacaoParcial}
    Seja $G(p, s)$ um símbolo de função introduzido por definição.
