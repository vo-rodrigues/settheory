...
\begin{proposition}[$\BST$](Esquema de boa ordenação)\label{prop:esquemaDaBoaOrdenacao}\index{esquema de boa ordenação}
...
\begin{proposition}[$\BST$](Esquema da indução transfinita)\label{prop:esquemaDaInducaoTransfinita}
    Seja $\phi(\alpha, t_1, \dots, t_n)$ uma fórmula.
...
\begin{definition}[$\BST$]\label{def:sequenciaTransfinita}\index{sequência transfinita}
...
Para se definir $\beta+\alpha$, é necessário ter o conhecimento prévio do valor de potencialmente todos os valores $\beta+\xi$ para $\xi <\alpha$.
...
O objeto resultante é um símbolo de função $+$ introduzido por definição ou, analogamente, uma regra de definição para introduzir tal símbolo.
...
\begin{example}[$\ZFmPI$]\label{exa:somaOrdinal}
...
\begin{definition}[$\BST$]\label{def:computacaoParcialTransfinita}
...
\begin{lemma}[$\BST$]\label{lem:compatibilidadeDeComputacoesParciais}
...
\begin{corollary}[$\BST$]\label{cor:unicidadeDeComputacaoParcial}
    Seja $G(p, s)$ um símbolo de função introduzido por definição.
...
\begin{lemma}[$\ZFmPI$]\label{lem:existenciaDeComputacoesParciais}
    Seja $G(p, s)$ um símbolo de função introduzido por definição.
...
\begin{definition}[$\ZFmPI$]\label{def:iteracaoTransfinita}
    Seja $G(p, s)$ um símbolo de função introduzido por definição.
...
\begin{theorem}[$\ZFmPI$]\label{thm:equacaoDaIteracaoTransfinita}
...
Isto é, qualquer outra função que satisfaça a mesma equação recursiva deve coincidir com $F$ em todos os ordinais, independentemente de como ela seja definida.

Novamente, a ferramenta apropriada para isso é a indução transfinita.
\begin{theorem}[$\ZFmPI$]\label{thm:unicidadeDaIteracaoTransfinita}
...
\end{proof}
Com isso, fica demonstrado o Metateorema~\ref{mthm:esquemaDaRecursaoTransfinita}.


\subsection*{Exercícios}

\begin{exercise}[$\BST$]
    Prove a seguinte versão finitária do Lema~\ref{lem:existenciaDeComputacoesParciais} em $\BST$, ou seja, sem utilizar o Axioma da Substituição.

    Seja $G(p, s)$ um símbolo de função introduzido por definição.
    Para todo $p$ e todo ordinal $n$ tal que $\nat(n)$, existe uma computação parcial de comprimento $n$ e parâmetro $p$ baseada em $G$.
    
    Observação: para a definição de $\nat(\alpha)$, veja o Exercício~\ref{exr:caracterizacaoDeNaturaisComoOrdinais}.
\end{exercise}

\begin{exercise}[$\BST$]
    Em continuação ao exercício anterior, conclua que vale em $\BST$ o seguinte esquema de recursão finitária.
    Denotemos por $V^{<\omega}$ o conjunto de todas as sequências cujo domínio é um ordinal $n$ tal que $\nat(n)$.

    Suponha que um símbolo de função binário $G(p, s)$ seja introduzido por definição.
    Então é possível introduzir um símbolo de função binário $F(p, n)$ de modo que
    \[
        \forall n \in \ON \forall p \Big( \nat(n)\rightarrow F_{p}(n)=G(p, F_p|n)\Big).
    \]
\end{exercise}

\begin{exercise}[$\BST$]
    Utilize o exercício anterior para mostrar que é possível definir soma, produto e exponenciação de números naturais mesmo sem o Axioma do Infinito.
    Conclua ainda que, em $\BST$, são válidas todas as propriedades usuais dessas operações (Exercício~\ref{exr:propriedadesDasOperacoesAritmeticas}).
\end{exercise}

\begin{exercise}[$\BST$]
    Utilize o lema anterior para mostrar que podemos definir, para $n$ ordinal tal que $\nat(n)$, e $f$ uma função, a $n$-ésima iteração de $f$, denotada por $f^n$, que satisfaça

    \begin{itemize}
        \item $f^0=\id_{\dom f}$;
        \item $f^{S(n)}=f\circ f^n$ para todo $n$ tal que $\nat(n)$;
    \end{itemize}

    Verifique que se $\ran f \subseteq \dom f$, então $\dom(f^n)=\dom f$ para todo $n$ tal que $\nat(n)$.

    Utilize isso para fornecer uma demonstração do Teorema de Cantor-Schröder-Bernstein em $\BST$ análoga à apresentada nesse texto.

    Conclua que são válidos em $\BST$ todos os resultados, definições e exercícios da Seção~\ref{sec:conjuntosFinitos} com excessão do item~\ref{item:omegaInfinito} do Corolário~\ref{cor:finitoCardinal}.
\end{exercise}