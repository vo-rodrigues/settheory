\section{Indução e Recursão transfinitas}
...
\begin{proposition}[Esquema de boa ordenação]\index{esquema de boa ordenação}
...
\begin{proposition}[Esquema da indução transfinita]
    Seja $\phi(\alpha, t_1, \dots, t_n)$ uma fórmula.
...
\begin{proof}
    ...
    Pelo esquema de boa ordenação, existe o menor ordinal $\alpha$ tal que $\neg\phi(\alpha, t_1, \dots, t_n)$.
    Assim, para todo $\beta <\alpha$, temos $\phi(\beta, t_1, \dots, t_n)$.
    Mas então $\phi(\alpha, t_1, \dots, t_n)$, o que é uma contradição.
\end{proof}

\begin{definition}\index{sequência transfinita}
...
\begin{metatheorem}[Esquema da recursão transfinita]\index{recursão transfinita}
    ...
    \]
\end{metatheorem}
Chamamos o resultado acima de metateorema porque ele não afirma apenas uma propriedade de conjuntos já dados na teoria: ele afirma que, a partir de um símbolo de função $G$ introduzido por definição, é possível introduzir um novo símbolo de função $F$ satisfazendo a equação recursiva desejada.
...
\begin{example}\label{ex:soma-ordinal}
...
Dedicaremos o resto da seção para a prova do metateorema acima.
...
A compatibilidade provada acima garantirá que essa escolha independe da computação parcial utilizada.
\begin{lemma}
    Seja $G(p, s)$ um símbolo de função introduzido por definição.
...
\begin{proof}
    ...
    Finalmente, se $\alpha$ é um ordinal limite, para cada $\beta <\alpha$, por hipótese de indução e pelo corolário anterior, existe uma única computação parcial $h_\beta$ de comprimento $\beta$ e parâmetro $p$ baseada em $G$.
    Pelo esquema da substituição, seja $H=\{h_\beta : \beta <\alpha\}$.
    
    Pelo lema, $H$ é um conjunto de funções compatíveis, logo, $h=\bigcup H$ é uma função de domínio $\bigcup_{\beta <\alpha} \beta=\alpha$.
...
\begin{definition}
    ...
    \]
\end{definition}
Pelos lemas, $F^G(p, \alpha)$ é bem definido, pois tal $y$ existe e é único.
