\section{Números ordinais}\label{sec:numerosOrdinais}
\index{ordinal}
\index{conjunto transitivo}
...
\[
    B=\{0, 1, 2, 3, 6\}
\]
Ambos são conjuntos bem ordenados por $\in$ (pois são totalmente ordenados por $\in$, e finitos, ver Corolário~\ref{prop:finitoBoaOrdem}), e são isomorfos, mas são distintos.
...
Conforme veremos, com a presença do Axioma da Substituição, que será introduzido na Seção~\ref{sec:axiomasDaSubstituicao}, conjuntos bem ordenados por $\in$ que satisfazem essa condição são exatamente os representantes de todas as boas ordens.

\begin{definition}[$\BST$]\label{def:conjuntoTransitivo}\index{conjunto transitivo}
    Um conjunto $A$ é dito \emph{transitivo} se, para todo $a\in A$, $a\subseteq A$.
\end{definition}

Agora podemos definir um ordinal.

\begin{definition}[$\BST$]\label{def:ordinal}\index{ordinal}
...
\begin{example}[$\BSTI$]\label{exa:omegaEhOrdinal}
    $\omega$ é um ordinal: pelo Corolário~\ref{cor:boaOrdemDosNaturais}, $\omega$ é um conjunto bem-ordenado por $\in$, e, para todo $a\in \omega$, $a$ é um número natural, e, portanto, $a\subseteq \omega$.
\end{example}

Além disso, todos os números naturais são ordinais pelo Exemplo~\ref{exa:omegaEhOrdinal} e pelo Lema~\ref{lem:elementosDeOrdinalSaoOrdinais}.

\begin{lemma}[$\BST$]\label{lem:elementosDeOrdinalSaoOrdinais}
...
\begin{lemma}[$\BST$]\label{lem:intersecaoNaoVaziaDeOrdinaisEhOrdinal}
...
\begin{lemma}[$\BST$]\label{lem:inclusaoDeOrdinaisEquivPertencimentoOuIgualdade}
...
\end{proof}
\begin{proposition}[$\BST$]\label{prop:estruturaBasicaDosOrdinais}
...
\begin{corollary}[$\BST$]\label{cor:conjuntoTransitivoDeOrdinaisEhOrdinal}
Todo conjunto de ordinais é bem-ordenado por $\in$.
Assim, um conjunto de ordinais transitivo é um ordinal.
\end{corollary}
\begin{corollary}[$\BST$]\label{cor:naoExisteConjuntoDeTodosOrdinais}
...
\begin{proposition}[$\BST$]\label{prop:sucessorDeOrdinalEhOrdinal}
    Seja $\alpha$ um ordinal.
...
$\ON$ é um símbolo de predicado unário, e escrevemos $\alpha \in \ON$ como uma outra forma de escrever $\ON(\alpha)$, que se traduz como ``$\alpha$ é um ordinal''.
Dessa forma, expressões como $\mathcal P(\ON)$ não são termos bem formados.


\begin{lemma}[$\BST$]\label{lem:uniaoDeOrdinaisEhSupremo}\index{supremo!de ordinais}
...
Pelo Lema~\ref{lem:uniaoDeOrdinaisEhSupremo}, quando $X$ é um conjunto de ordinais, podemos escrever $\sup X$ para $\bigcup X$.

Nem todo ordinal é o sucessor de outro ordinal.
Por exemplo, $\omega$ não é o sucessor de nenhum ordinal.

\begin{definition}[$\BST$]\label{def:ordinaisSucessoresELimites}\index{ordinal!sucessor}\index{ordinal!limite}
...
\begin{proposition}[$\BSTI$]\label{prop:omegaEhOrdinalLimite}
...
\begin{proposition}[$\BST$]\label{prop:ordinaisNaoSaoIsomorfos}
...
\begin{proof}
    ...
    Temos que $\alpha$ é um segmento inicial próprio de $\beta$: pelo Lema~\ref{lem:inclusaoDeOrdinaisEquivPertencimentoOuIgualdade}, $\alpha\subsetneq \beta$, e, se $\xi<\delta$ para algum $\delta\in \alpha$, então $\xi\in \alpha$.
    Dessa forma, $\alpha$ é um segmento inicial próprio de $\beta$.

    Pelo Lema~\ref{lem:isomorfismoParaSegmentoInicialEhIdentidade}, $\alpha$ não é isomorfo à $\beta$.
\end{proof}
...
\begin{exercise}[$\BST$]
    ...
    \]
\end{exercise}
\begin{exercise}[$\BST$]\label{exr:segmentoInicialDeOrdinal}
...
\end{exercise}
\begin{exercise}[$\BST$]
    Seja $X$ um conjunto de ordinais não vazio sem máximo.
    Prove que $\bigcup X$ é um ordinal limite.
\end{exercise}

\begin{exercise}[$\BST$]\label{exr:caracterizacaoDeNaturaisComoOrdinais}
    Vamos definir $\nat(n)$ como a propriedade que diz que $n$ é um ordinal e, para todo $m\leq n$, $n$ não é um ordinal limite.

    \begin{enumerate}[label=(\alph*)]
        \item Prove que $\nat(0)$ e que se $\nat(n)$, então $\nat(S(n))$.
        \item Assuma $\BSTI$.
        Prove que, para todo $x$, $x \in \omega \iff \nat(x)$.
    \end{enumerate}

    Assim, é possível falar em números naturais mesmo sem assumir o Axioma do Infinito.
\end{exercise}