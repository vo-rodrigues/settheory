...
\[
    B=\{0, 1, 2, 3, 6\}
\]
Ambos são conjuntos bem ordenados por $\in$ (pois são finitos, ver Corolário~\ref{cor:finitoBoaOrdem}), e são isomorfos, mas são distintos.
...
Formalmente, $\ON$ é uma relação unária, e escrevemos $\alpha \in \ON$ como uma outra forma de escrever $\ON(\alpha)$, que se traduz como ``$\alpha$ é um ordinal''.
