...
$\ON$ é uma relação unária, e escrevemos $\alpha \in \ON$ como uma outra forma de escrever $\ON(\alpha)$, que se traduz como ``$\alpha$ é um ordinal''.
...
\end{exercise}
\begin{exercise}
    Mostre que para todo ordinal $\beta$, $\bigcup(\beta+1)=\beta$.
\end{exercise}